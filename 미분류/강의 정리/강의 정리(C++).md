...
### 함수 템플릿
    - 함수의 일반화 서술 : 구체적인 데이터형을 포괄할 수 있는 일반형으로 함수를 정의
    - 함수 오버로딩 작업을 자동화하는 것
    - 프로그램을 짧게 만들어주는게 아닌 컴파일러가 수동으로 정의 할때 와 마찬가지로 별개의 함수 정의가 생성됨
    - 함수의 시그내처가 다르면 오버로딩이 가능

- 명시적 특수화
    - 컴파일러가 함수 호출에 정확히 대응하는 특수화된 정의를 발견하면, 템플릿을 찾지 않고 정의를 사용
    - template <> void Swap<name>(name &, name &);

- 명시적 구체화
    - 컴파일러가 특정 데이터형에 맞는 함수 정의를 생성하기 위해 템플릿을 사용할 때, 그 결과

- auto와 decltype
    - auto : 값에 상응하는 타입을 추론시켜주는 키워드
    - decltype : 값으로부터 타입을 추출해 낼 수 있는 키워드
...
### 자동 기억 존속 시간(automatic storage duration) : 2가지
    프로그램이 실행될 떄 생기고 함수나 블록을 떠날 떄 해제
    자동 변수는 Stack에 저장, Stack은 프로그램 실행시간 동안 메모리 크기가 
    유동적으로 증감

#### 자동변수 ( 지역 변수 )
    프로그램 실행 동안 늘어나고 줄어드는 자동변수
    메모리 일부를 예약하거나, 생성 소멸을 '스택으로 관리'

    esp, ebp라는 포인터로 시작 위치인 스택의 바닥, 다음 데이터 저장을 위해 비어있는 스택의 꼭대기를 지시, 이동을 반복 지시해 스택의 크기를 관리

#### 레지스터 변수
    메모리가 아니라 CPU안에 레지스터에 저장해 연산을 빠르게 수행하기 위한 변수
    많은 반복에 유리

    
### 정적 기억 존속 시간(static storage duration) : 3가지
     함수 바깥에서 정의된 변수, static을 사용한 변수

#### 정적 변수 ( 전역 변수 )
    프로그램이 실행되는 동안 데이터 블록에 저장되어짐
    명시적으로 초기화하지 않으면 0으로 초기화
    내부 블록에서 static을 사용해 정적 변수를 만들 수 있음

    블록안의 static 선언한 변수는 존속시간이 길어지고
    함수 밖에서 선어노딘 static 변수는 내부 링크를 의미

    ** 내부링크 외부링크(둘다 정적 존속 기간)


### 쓰레드 존속 시간(Thread Storage Duration) : 
    해당 쓰레드가 존속되는 전체 실행시간동안 유지되는 시간 영역

### 동적 기억 존속 시간(dynamic storage duration) :
    자유 공간 혹은 Heap이 해당되는 영역
    new를 사용해 대입, delete로 해제되거나 프로그램이 종료할 때까지

### 사용 범위(Scope) 와 링크 
    - 사용범위 : 어떤 이름이 한의 파일 안에서 얼마나 알려져있는지, 쓸수있는지
    - 링크 : 다른 파일들이 이름을 공유하는 것
